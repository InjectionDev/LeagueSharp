                ...
                Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.LaneClear)
            {
                ...
                if (target.IsMinion)
                {
                    var MinionList = MinionManager.GetMinions(Player.Position, Q.Range, MinionTypes.All, MinionTeam.Enemy, MinionOrderTypes.Health)
                        .Where(x => !x.IsDead && target.ServerPosition.Distance(x.ServerPosition) > 3 && HealthPrediction.LaneClearHealthPrediction(x, (int)(Player.AttackDelay * 1000 * 1.1)) <= 0).ToList();
