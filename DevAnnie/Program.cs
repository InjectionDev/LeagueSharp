        ...
        static void AssemblyUtil_onGetVersionCompleted(OnGetVersionCompletedArgs args)
        {
            if (args.IsSuccess)
            {
                if (args.CurrentVersion == Assembly.GetExecutingAssembly().GetName().Version.ToString())
                    Game.PrintChat(string.Format("<font color='#fb762d'>DevAnnie You have the lastest version. {0}</font>", Assembly.GetExecutingAssembly().GetName().Version));
                else
                    Game.PrintChat(string.Format("<font color='#fb762d'>DevAnnie NEW VERSION available! Tap F8 for Update! {0}</font>", args.CurrentVersion));
            }
        }
